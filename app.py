...
if prompt := st.chat_input("Ask me to create a dataset, program, etc..."):
    ...
    with st.chat_message("assistant"):
        # Check size before including metadata context
        metadata = st.session_state.fetched_metadata
        if metadata and len(metadata) > 500:
            st.markdown(f"⚠️ The metadata set contains {len(metadata)} items, too large to include in context.")
            buffer = io.StringIO(json.dumps(metadata, indent=2))
            st.download_button("📥 Download Full Metadata JSON", buffer, "dhis2_metadata.json", "application/json")
        else:
