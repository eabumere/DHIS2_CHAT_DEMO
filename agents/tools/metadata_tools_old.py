...
def generate_dhis2_ids(count: int = 1) -> str:
    """
    Generate one or more unique DHIS2 IDs using the /api/system/id endpoint.
    Example: generate_dhis2_ids(3) will return 3 valid UIDs.
    """
    if not all([base_url, username, password]):
        return "❌ Missing DHIS2 credentials"
...
def get_dhis2_version() -> str:
    """
    Returns the version of the connected DHIS2 instance.
    """
    if not all([base_url, username, password]):
        return "❌ Missing DHIS2 credentials"
    try:
...
@tool
def get_play_metadata_template_by_version(
        version: str,
        metadata_type: str,
    ...
    """
    Fetch a metadata structure from a DHIS2 Play instance (e.g., dataSets, programs) using version and optional filter.

    Parameters:
    - version: DHIS2 version string (e.g., "2.40")
    - metadata_type: metadata endpoint (e.g., "dataSets", "programs", "organisationUnits")
    - filter_field: field to filter on (e.g., "periodType")
    - filter_value: expected value for the filter field (e.g., "Monthly")

    Returns:
    - A single metadata object from the play instance in JSON format
    """
    try:
        base = f"https://play.dhis2.org/{version}/api/{metadata_type}.json"
        params = {"paging": "false", "fields": "id,name"}
        if filter_field:
            params["fields"] += f",{filter_field}"
        response = requests.get(base, auth=("admin", "district"), params=params)
        response.raise_for_status()
        items = response.json().get(metadata_type, [])

        match = next((item for item in items if item.get(filter_field) == filter_value), None) if filter_field and filter_value else (items[0] if items else None)

        if not match:
            return f"⚠️ No {metadata_type} found in DHIS2 {version} Play instance matching your criteria."

        details = requests.get(
            f"https://play.dhis2.org/{version}/api/{metadata_type}/{match['id']}.json",
            auth=("admin", "district"),
            params={"fields": "*"}
        )
        details.raise_for_status()
        return json.dumps(details.json(), indent=2)
    except Exception as e:
        return f"❌ Failed to fetch {metadata_type} from Play version {version}: {str(e)}"
...
def dhis2_tool_metadata_wrapper(input_str: str) -> dict:
    try:
        ...
        for item in payload:
            ...
            # Auto-fill known required fields
            for field in required_fields:
                if field not in item:
                    if field == "shortName" and "name" in item:
                        short = item["name"][:45]  # Truncate
                        item["shortName"] = short + generate_suffix()

            still_missing = [f for f in required_fields if f not in item]
            if still_missing:
                return {
                    "status": "error",
                    "reason": f"Missing required fields: {', '.join(still_missing)}",
                    "missingFields": still_missing,
                    "schema": f"/api/schemas/{schema_name}.json"
                }
...
@tool
def validate_metadata_payload(input_str: str) -> dict:
    """
    Validates metadata payload structure using the DHIS2 schema.
    ...
    Returns validation result, auto-fills shortName if missing, and shows any missing fields.
    """
    try:
        data = json.loads(input_str)
        endpoint = data["endpoint"]
        ...
        validated = []
        for item in payload:
            # Apply known defaults for some schema types
            if schema_name == "dataElement":
                item.setdefault("aggregationType", "SUM")
                item.setdefault("categoryCombo", {"id": "bjDvmb4bfuf"})
                item.setdefault("zeroIsSignificant", False)
            ...
            for field in required_fields:
                if field not in item and field == "shortName" and "name" in item:
                    item["shortName"] = item["name"][:45] + generate_suffix()
...
def validate_object_against_schema(obj: Any, schema: dict, path="") -> List[str]:
    """
    Recursively validate an object against the schema.
    Return list of missing required field paths (empty if valid).
    """
    missing_fields = []
    ...
    for field in required_fields:
        if field not in obj:
            ...
            # If property is present and is an object or array, validate recursively
                ...
                if prop_schema.get("type") == "array":
                    ...
                    if isinstance(val, list):
                        for i, item in enumerate(val):
                            nested_missing = validate_object_against_schema(item, items_schema, path=f"{path}{field}[{i}].")
                            missing_fields.extend(nested_missing)
                    else:
                        missing_fields.append(f"{path}{field} (expected array)")
                # Handle nested objects
                elif prop_schema.get("type") == "object":
                    if isinstance(val, dict):
                        nested_missing = validate_object_against_schema(val, prop_schema, path=f"{path}{field}.")
                        missing_fields.extend(nested_missing)
                    else:
                        missing_fields.append(f"{path}{field} (expected object)")
                # For other types (string, boolean, etc) no recursive validation needed

    return missing_fields


@tool
def validate_metadata_payload(payload_str: str) -> str:
    """
    Validate metadata JSON payload string for required fields recursively.
    Return missing fields or success message.
    """
    try:
    ...
    if not isinstance(payload, dict):
        return "❌ Payload should be a JSON object at top level."

    # Get the root key (e.g. dataSets, dataElements, categoryCombos, etc)
    ...
    if not isinstance(items, list):
        return f"❌ Expected an array for '{root_key}'."

    # Guess schema name (singular) from root_key
    # Simple heuristic: remove trailing 's' or 'es' for plurals
    if root_key.endswith("ies"):
        schema_name = root_key[:-3] + "y"
    elif root_key.endswith("es"):
        schema_name = root_key[:-2]
    elif root_key.endswith("s"):
        schema_name = root_key[:-1]
    else:
    ...
    # Validate each item recursively against schema
    all_missing_fields = []
    for i, item in enumerate(items):
        missing = validate_object_against_schema(item, schema, path=f"{root_key}[{i}].")
        all_missing_fields.extend(missing)

    if all_missing_fields:
        missing_str = "\n".join(all_missing_fields)
        return f"❌ Missing required fields:\n{missing_str}\nSchema reference: /api/schemas/{schema_name}.json"

    return f"✅ Payload is valid for schema `{schema_name}`."
